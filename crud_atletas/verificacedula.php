...
try {
    ...
    $check = $db->prepare("SELECT id FROM atletas WHERE cedula = ? UNION SELECT id FROM atletas WHERE cedula = ?");
    ...
    if ($check->fetch()) {
        ...
        // 1. Verificar si ya existe en atletas o atletas_tmp
    $db = (new Database())->getConnection();
    $check = $db->prepare("SELECT id FROM atletas_tmp WHERE cedula = ? UNION SELECT id FROM atletas_tmp WHERE cedula = ?");
    $check->execute([$cedula, $cedula]);

    if ($check->fetch()) {
        echo json_encode(['success' => false, 'message' => '⚠️ La cédula ya está registrada en el atletas_temp']);
        exit;
    }
