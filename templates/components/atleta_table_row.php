...
<tr>
    ...
    <td class="fvd-col-sol-carnet">
        <?php if ($mostrarSolCarnet): ?>
            <a class="fvd-atleta-sol-carnet" href="<?= htmlspecialchars($fvd_url_solicitud_carnet_base . '?atleta_id=' . $aid, ENT_QUOTES, 'UTF-8') ?>">Solicitar carnet</a>
        <?php else: ?>
            <?= $dash ?>
        <?php endif; ?>
    </td>
    ...
    <td class="fvd-col-actions fvd-col-actions--icons">
        <span class="fvd-action-icons" role="group" aria-label="Acciones">
            ...
            <?php endif; ?>
            <?php if ($esBaja): ?>
                <form method="post" action="<?= htmlspecialchars($selfUrl, ENT_QUOTES, 'UTF-8') ?>" style="display:inline;margin:0" onsubmit="return confirm('¿Restaurar este atleta al listado (estatus pendiente)?');">
            ...
            <?php else: ?>
                <form method="post" action="<?= htmlspecialchars($selfUrl, ENT_QUOTES, 'UTF-8') ?>" style="display:inline;margin:0" onsubmit="return confirm('¿Dar de baja al atleta? No se borrará el registro.');">
            ...
            <?php endif; ?>
            <a class="fvd-icon-btn fvd-action-ficha-only" href="<?= htmlspecialchars($selfUrl . '?action=carnets&ids=' . $aid, ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener" title="Solicitud de carnet">
