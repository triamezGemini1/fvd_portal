...
<form class="fvd-mod-form fvd-mod-form--wide fvd-mod-form--asoc-2col" method="post" enctype="multipart/form-data" action="">
    ...
    <div class="fvd-row fvd-row--span2">
        ...
        <?php if (!empty($r['logo'])): ?>
            <span class="fvd-atl-muted" style="display:block;margin-top:4px">Actual: <?= htmlspecialchars((string) $r['logo'], ENT_QUOTES, 'UTF-8') ?>
            <?php if (is_string($r['logo']) && $r['logo'] !== ''): ?>
                — <a href="<?= htmlspecialchars(upload_url($r['logo']), ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener">ver</a>
            <?php endif; ?>
            </span>
        <?php endif; ?>
    </div>
