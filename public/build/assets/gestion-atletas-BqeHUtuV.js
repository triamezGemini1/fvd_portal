        ...
        <div class="fvd-gestion-atletas-sticky sticky top-0 z-40 shrink-0 border-b border-black/35 bg-gradient-to-br from-[#060d22] via-[#0c1a3d] to-[#122a4a] text-slate-100 shadow-[0_8px_32px_rgba(0,0,0,0.45)]">
          <div class="mx-auto w-full max-w-[min(100%,76.8rem)] px-3 sm:px-4">
            ...
            <div class="border-b border-white/10 py-3">
              <label class="mb-1.5 block text-[9px] font-semibold uppercase tracking-[0.22em] text-slate-500" for="fvd-atleta-q">Búsqueda inteligente</label>
              <input type="search" id="fvd-atleta-q" placeholder="Cédula · email · nombre…" value="${r(u.q)}" autocomplete="off" class="min-h-[2.65rem] w-full min-w-0 rounded-md border-0 bg-white/10 px-3 py-2.5 text-sm text-white shadow-inner outline-none ring-1 ring-inset ring-white/15 placeholder:text-slate-500 focus:bg-white/14 focus:ring-2 focus:ring-[#fff200]/45" />
              <div class="mt-2 flex w-full min-w-0 flex-wrap items-stretch gap-2">
                ${t.isFvdAdmin?`<select id="fvd-atleta-asoc" class="fvd-atleta-select-header min-h-[2.5rem] min-w-0 flex-1 rounded-md border border-slate-300 bg-white px-2 py-2 text-xs font-semibold text-slate-900 shadow-md outline-none focus-visible:border-[#2e3092] focus-visible:ring-2 focus-visible:ring-[#fff200]/60 sm:min-w-[10.5rem] sm:flex-none sm:text-sm">${j}</select>`:``}
                <select id="fvd-atleta-tipo" class="fvd-atleta-select-header min-h-[2.5rem] min-w-0 flex-1 rounded-md border border-slate-300 bg-white px-2 py-2 text-xs font-semibold text-slate-900 shadow-md outline-none focus-visible:border-[#2e3092] focus-visible:ring-2 focus-visible:ring-[#fff200]/60 sm:min-w-[8.5rem] sm:flex-none sm:text-sm">
            ...
            <div class="border-b border-white/10 py-2.5">
              <div class="w-full min-w-0 sm:w-1/2 sm:max-w-[50%]">
                <div class="flex flex-wrap items-end gap-2 gap-y-2">
                  <div class="flex min-w-0 flex-1 flex-wrap gap-2">
                    <div class="rounded-lg border border-white/10 bg-black/25 px-2 py-1.5 shadow-inner sm:px-2.5 sm:py-2">
                      <p class="text-[8px] font-bold uppercase tracking-widest text-slate-500">Afiliados (filtro)</p>
                      <p class="text-sm font-bold tabular-nums leading-tight text-white sm:text-base">${u.total}</p>
                    </div>
                    <div class="min-w-0 max-w-[9rem] flex-1 rounded-lg border border-white/10 bg-black/25 px-2 py-1.5 shadow-inner sm:max-w-[11rem] sm:px-2.5 sm:py-2">
                      <p class="text-[8px] font-bold uppercase tracking-widest text-slate-500">Asociación</p>
                      <p class="truncate text-[10px] font-semibold leading-snug text-slate-100 sm:text-[11px]" title="${r(P)}">${r(P)}</p>
                    </div>
                    <div class="rounded-lg border border-white/10 bg-black/25 px-2 py-1.5 shadow-inner sm:px-2.5 sm:py-2">
                      <p class="text-[8px] font-bold uppercase tracking-widest text-slate-500">Vista</p>
                      <p class="text-[10px] font-semibold text-slate-100 sm:text-[11px]">${r(ie)}</p>
                    </div>
                  </div>
                  <span id="fvd-atleta-loading-dot" class="${u.loading?`opacity-100 animate-pulse`:`opacity-0`} mb-0.5 inline-block h-2 w-2 shrink-0 self-center rounded-full bg-amber-300 shadow transition-opacity duration-150" aria-hidden="true"></span>
                </div>
              </div>
            </div>
