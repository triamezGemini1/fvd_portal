/* empty css            *//* empty css                  */var e=typeof window<`u`&&window.FVD_GESTION_ATLETAS?window.FVD_GESTION_ATLETAS:{apiUrl:``},t=(e.referencialUrl||``).trim(),n=e.embeddedMaster===!0;function r(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}function i(e){let t=e.trim();if(!t)return`?`;let n=t.split(/\s+/).filter(Boolean);return n.length>=2?(n[0][0]+n[n.length-1][0]).toUpperCase():t.slice(0,2).toUpperCase()}async function a(e,t){let n=await fetch(e,{credentials:`include`,...t}),r;try{r=await n.json()}catch{r={ok:!1,error:`Respuesta no JSON.`}}return{res:n,body:r}}function o(e,t){let n=Number(e);return Number(t)===0?`Pendiente`:n===9?`Inactivo`:n===0||n===1?`Activo`:n===2?`Baja`:n===990?`Pend. adm.`:`—`}function s(e){let t=String(e||``).trim();if(!t)return`— (indique fecha de nac.)`;let n=t.split(`-`);if(n.length!==3)return`—`;let r=parseInt(n[0],10),i=parseInt(n[1],10)-1,a=parseInt(n[2],10);if(Number.isNaN(r)||Number.isNaN(i)||Number.isNaN(a))return`—`;let o=new Date(r,i,a),s=new Date;if(s.setHours(0,0,0,0),o.setHours(0,0,0,0),o>s)return`—`;let c=s.getFullYear()-o.getFullYear(),l=s.getMonth()-o.getMonth();return(l<0||l===0&&s.getDate()<o.getDate())&&--c,c>=18?`LIBRE`:c>=15?`SUB 18`:c>=12?`SUB 15`:`SUB 12`}function c(t,n,r){let i=(e.reportsBase||``).trim();if(!i)return`#`;let a=i.endsWith(`/`)?i:`${i}/`,o;try{o=new URL(t.replace(/^\//,``),a)}catch{return`#`}let s=new URL(window.location.href);return[`embedded`,`fvd_master_embed`,`torneo_id`,`ctx_torneo`,`campeonato_id`,`ret`,`return`,`fvd_ws`].forEach(e=>{let t=s.searchParams.get(e);t!==null&&t!==``&&o.searchParams.set(e,t)}),n&&r>0&&o.searchParams.set(`asociacion_id`,String(r)),o.searchParams.has(`ret`)||o.searchParams.set(`ret`,s.pathname+(s.search||``)),o.pathname+o.search}function l(e,t){if(!e)return``;let n=e?.foto_url?String(e.foto_url):``,a=n?`<img src="${r(n)}" alt="" class="mx-auto h-10 w-10 rounded-lg object-cover ring-1 ring-slate-200/80 shadow-sm" />`:`<span class="mx-auto flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-[10px] font-bold text-slate-500">${r(i(String(e?.nombre??``)))}</span>`,s=Number(e?.numfvd??0),c=s>0?String(s):`—`,l=e?.cedula!=null&&String(e?.cedula).trim()!==``?String(e?.cedula):`—`,u=r(l),d=`<div class="flex max-w-[13rem] items-center gap-1.5 whitespace-nowrap text-left text-[13px] font-semibold leading-tight text-slate-800" title="${e?.identidad_label!=null&&String(e?.identidad_label).trim()!==``?r(String(e?.identidad_label).replace(/\n/g,` · `)):`C.I. ${u}`}">
      ...
      <form id="fvd-atleta-form" class="fvd-atleta-create-form max-h-[min(78dvh,85vh)] w-full max-w-full overflow-y-auto rounded-2xl border-2 border-[#fff200]/45 bg-gradient-to-br from-[#060d22] via-[#0f1f4a] to-[#1a2d6e] p-4 text-[0.9375rem] font-sans leading-relaxed text-[#eef2ff] antialiased shadow-[0_24px_64px_rgba(0,0,0,0.45)] sm:p-5">
        ...
        ${t.isFvdAdmin?`<label class="block min-w-0">
            ...
            <select name="asociacion" ${d.modal===`create`?`required`:``} class="${p}" ${f}>
              <option value="">${d.modal===`create`?`— Seleccione asociación —`:`—`}</option>
              ${(t.asociaciones||[]).map(e=>`<option value="${e.id}" ${Number(g.asociacion)===Number(e.id)?`selected`:``}>${r(e.nombre||``)}</option>`).join(``)}
            </select>
          </label>`:`<input type="hidden" name="asociacion" value="${r(String(g.asociacion??``))}" />`}
            ...
            <div class="grid grid-cols-1 items-end gap-x-8 gap-y-8 sm:grid-cols-[minmax(0,2fr)_minmax(0,5.5fr)_minmax(0,2.5fr)]">
              <div class="min-w-0">
                <label class="block min-w-0">
                  ...
                  <input name="cedula" id="fvd-atleta-cedula" required class="${p}" value="${r(String(g.cedula??``))}" ${f} />
                </label>
                ${E?`<p class="mt-1 text-xs font-normal leading-snug text-[#c5d4ef]" role="status">${r(E)}</p>`:``}
              </div>
              <label class="block min-w-0">
                <span class="${m}">Nombre completo</span>
                <input name="nombre" id="fvd-atleta-nombre" required class="${p}" value="${r(w)}" ${f} />
              </label>
              <label class="block min-w-0">
                <span class="${m}">Sexo</span>
                <select name="sexo" class="${p}" ${f}>
                  <option value="0" ${Number(g.sexo)===0?`selected`:``}>— No indicado —</option>
                  <option value="1" ${Number(g.sexo)===1?`selected`:``}>Masculino</option>
                  <option value="2" ${Number(g.sexo)===2?`selected`:``}>Femenino</option>
                </select>
              </label>
            </div>
            ...
            <div class="grid grid-cols-1 items-end gap-x-8 gap-y-8 sm:grid-cols-[minmax(0,2.5fr)_minmax(0,3fr)_minmax(0,4.5fr)]">
              <label class="block min-w-0">
                ...
                <input type="date" name="fechnac" id="fvd-atleta-fechnac" class="${p}" value="${r(String((g.fechnac||``).toString().slice(0,10)))}" ${f} />
              </label>
              <label class="block min-w-0">
                <span class="${m}">Profesión</span>
                <input name="profesion" class="${p}" value="${r(String(g.profesion??``))}" ${f} />
              </label>
              <label class="block min-w-0 sm:col-span-1">
                <span class="${m}">Dirección</span>
                <input name="direccion" class="${p}" value="${r(String(g.direccion??``))}" ${f} />
              </label>
            </div>
            ...
            <div class="grid grid-cols-2 items-end gap-x-8 gap-y-8 sm:grid-cols-4">
              <label class="min-w-0">
                ...
                <input name="celular" class="${p}" value="${r(String(g.celular??``))}" ${f} />
              </label>
              <label class="min-w-0">
                <span class="${m}">Email</span>
                <input type="email" name="email" class="${p}" value="${r(String(g.email??``))}" ${f} />
              </label>
              <div class="min-w-0">
                <span class="${m}">Nº FVD</span>
                <div class="flex min-h-[2.125rem] items-center rounded-lg border-2 border-[#fff200]/50 bg-[#050b1f]/95 px-2.5 text-sm font-semibold tabular-nums tracking-tight text-white">${Number(g.numfvd)>0?r(String(g.numfvd)):`—`}</div>
              </div>
              <div class="min-w-0">
                <span class="${m}">Categoría</span>
                <p id="fvd-atleta-categ-hint-txt" class="m-0 flex min-h-[2.125rem] items-center rounded-lg border border-white/30 bg-[#070f24]/95 px-2.5 text-sm font-semibold leading-snug tracking-tight text-[#fff566]">${r(ae)}</p>
              </div>
            </div>
          ...
          <aside class="flex min-w-0 flex-col items-center gap-2 lg:items-stretch" aria-label="Foto y cédula">
            <div class="flex w-full flex-col items-center gap-1">
              <div class="relative flex aspect-[3/4] max-h-[4.5rem] w-full max-w-[5.25rem] shrink-0 items-center justify-center self-center overflow-hidden rounded-md border border-white/25 bg-[#040a18]/80 p-1 shadow-inner">
                <img id="fvd-atleta-foto-prev" src="${C?r(_):``}" alt="" class="max-h-full max-w-full rounded object-contain ${C?``:`hidden`}" decoding="async" />
                <span id="fvd-atleta-foto-ph" class="px-1 text-center text-xs font-semibold tabular-nums text-[#8fa3cf] ${C?`hidden`:``}">${ce}</span>
              </div>
            ...
            <div class="flex w-full flex-col items-center gap-1">
              <div class="relative flex min-h-[3rem] w-full max-w-[6rem] shrink-0 items-center justify-center self-center overflow-hidden rounded-md border border-white/20 bg-[#040a18]/70 p-1">
                ...
                <span id="fvd-atleta-cedula-ph" class="px-1 text-center text-xs font-medium text-[#8fa3cf]">—</span>
              </div>
    ...
    `:``,ue=g?le:`<p class="py-6 text-center text-sm font-bold text-[#2e3092]">Cargando…</p>`,de=d.tipo===`bajas`?`Bajas`:d.tipo===`no_activos`?`Pendientes`:d.tipo===`ultimos`?`Últimos mov.`:`Activos`,F=`Todas`;if(t.isFvdAdmin&&d.asociacionFilter>0&&Array.isArray(t.asociaciones)){let e=t.asociaciones.find(e=>Number(e.id)===d.asociacionFilter);F=e&&e.nombre?String(e.nombre):`Asoc. #`+d.asociacionFilter}else !t.isFvdAdmin&&Array.isArray(t.asociaciones)&&t.asociaciones[0]&&(F=String(t.asociaciones[0].nombre||`Mi asociación`));let fe=d.rows.map(e=>l(e,t)).join(``),I=d.modal!==null,pe=d.modal===`create`?`Nuevo atleta`:d.modal===`edit`?`Editar atleta`:d.modal===`view`?`Ficha atleta`:``,me=t.canCreate&&!I?`<button type="button" id="fvd-atleta-nuevo" class="inline-flex h-8 shrink-0 items-center justify-center whitespace-nowrap rounded-md bg-[#fff200] px-2.5 text-[10px] font-extrabold uppercase tracking-wide text-[#0b1535] shadow-md ring-1 ring-black/10 transition hover:brightness-105 sm:h-9 sm:px-3 sm:text-xs">Nuevo atleta</button>`:``,L=(e.returnUrl||``).trim(),R=`inline-flex h-8 shrink-0 items-center justify-center gap-0.5 whitespace-nowrap rounded-md border border-white/30 bg-white/10 px-2 text-[10px] font-extrabold uppercase tracking-wide text-white shadow hover:bg-white/20 sm:h-9 sm:gap-1 sm:px-2.5 sm:text-xs`,z=`${n||!L?``:`<a href="${r(L)}" class="${R}">← Regresar</a>`}${n||L?``:`<button type="button" id="fvd-atleta-regresar" class="${R}">← Regresar</button>`}${me}`,he=z.trim()===``?``:`<div class="flex min-w-0 flex-nowrap items-center justify-end gap-1.5 overflow-x-auto border-b border-white/15 py-2 sm:gap-2">${z}</div>`,ge=d.error?`<div class="border-t border-white/10 py-2"><div class="rounded-md border border-red-400/40 bg-red-950/40 px-3 py-2 text-xs font-medium text-red-100 shadow-inner" role="alert">${r(d.error)}</div></div>`:``,B=e.reportsEnabled===!0&&(e.reportsBase||``).trim()!==``,_e=B?c(`reporte_indicadores.php?marcador=afiliacion`,t.isFvdAdmin,d.asociacionFilter):`#`,ve=B?c(`reporte_carnets.php`,t.isFvdAdmin,d.asociacionFilter):`#`,ye=B?c(`reporte_indicadores.php?marcador=afiliacion_anualidad`,t.isFvdAdmin,d.asociacionFilter):`#`,be=B?c(`reporte_traspasos.php`,t.isFvdAdmin,d.asociacionFilter):`#`,xe=B?`<div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1.5 border-t border-white/10 pt-2" role="navigation" aria-label="Informes HTML">
          <span class="w-full text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-500 sm:mr-1 sm:w-auto">Informes</span>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(_e)}" title="Filas con atletas.afiliacion = 1">Afiliación</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(ve)}" title="Solo filas con atletas.carnet = 1">Carnets</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(ye)}" title="Afiliación y anualidad">Afil.+anual.</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(be)}" title="Historial de traspasos">Traspasos</a>
        </div>`:``,V=u.querySelector(`#fvd-atleta-q`);V instanceof HTMLInputElement&&document.activeElement===V&&(b=!0,x=V.selectionStart??V.value.length,S=V.selectionEnd??V.value.length);let Se=`
              ...
              <div class="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                ...
                <div class="flex w-full min-w-0 flex-1 flex-row flex-wrap items-center gap-x-2 gap-y-1 sm:w-1/2 sm:max-w-[50%] sm:justify-end">
                  ...
                  <span class="inline-flex items-center gap-1.5 rounded-md border border-white/15 bg-black/30 px-2 py-0.5 shadow-inner">
                    ...
                    <span class="text-xs font-semibold leading-none text-slate-100">${r(de)}</span>
                  </span>
          ...
          <div class="fvd-atleta-tbody-scroll min-h-[min(12rem,calc(100dvh-17.5rem))] flex-1 overflow-auto rounded-b-lg border border-t-0 border-slate-300/90 bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]" style="max-height:calc(100dvh - 18.25rem)">
            <table class="min-w-[28.8rem] w-full table-fixed border-collapse text-left text-sm">
              ...
              <tbody class="divide-y divide-slate-100 bg-white text-slate-800">
                ${d.loading?`<tr><td colspan="6" class="px-3 py-12 text-center text-sm text-slate-500">Actualizando listado…</td></tr>`:d.rows.length===0?`<tr><td colspan="6" class="px-3 py-12 text-center text-sm text-slate-500">Sin resultados.</td></tr>`:fe}
              </tbody>
            </table>
          </div>
      ...
      ${I?`<div class="absolute inset-0 z-[99999] flex items-center justify-center p-3 sm:p-5" role="dialog" aria-modal="true">
        ...
        <div class="relative z-10 flex max-h-[min(92dvh,90vh)] w-[90%] max-w-[90%] flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
          <header class="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-[#1a1f5c] via-[#2e3092] to-[#3d42c4] px-4 py-3 sm:px-5">
            <div class="min-w-0">
              <h2 class="text-lg font-bold tracking-tight text-white">${r(pe)}</h2>
          ...
          <footer class="flex shrink-0 flex-wrap justify-end gap-2 border-t border-slate-100 bg-white px-4 py-3 sm:px-6">
            ${d.modal===`view`&&g&&d.modalCanEdit?`<button type="button" id="fvd-atleta-to-edit" class="rounded-lg bg-[#2e3092] px-4 py-2 text-sm font-bold text-white shadow hover:bg-[#252f7a]">Editar</button>`:``}
