/* empty css            *//* empty css                  */var e=typeof window<`u`&&window.FVD_GESTION_ATLETAS?window.FVD_GESTION_ATLETAS:{apiUrl:``},t=(e.referencialUrl||``).trim(),n=e.embeddedMaster===!0;function r(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}function i(e){let t=e.trim();if(!t)return`?`;let n=t.split(/\s+/).filter(Boolean);return n.length>=2?(n[0][0]+n[n.length-1][0]).toUpperCase():t.slice(0,2).toUpperCase()}async function a(e,t){let n=await fetch(e,{credentials:`include`,...t}),r;try{r=await n.json()}catch{r={ok:!1,error:`Respuesta no JSON.`}}return{res:n,body:r}}function o(e,t){let n=Number(e);return Number(t)===0?`Pendiente`:n===9?`Inactivo`:n===0||n===1?`Activo`:n===2?`Baja`:n===990?`Pend. adm.`:`—`}function s(e){let t=String(e||``).trim();if(!t)return`— (indique fecha de nac.)`;let n=t.split(`-`);if(n.length!==3)return`—`;let r=parseInt(n[0],10),i=parseInt(n[1],10)-1,a=parseInt(n[2],10);if(Number.isNaN(r)||Number.isNaN(i)||Number.isNaN(a))return`—`;let o=new Date(r,i,a),s=new Date;if(s.setHours(0,0,0,0),o.setHours(0,0,0,0),o>s)return`—`;let c=s.getFullYear()-o.getFullYear(),l=s.getMonth()-o.getMonth();return(l<0||l===0&&s.getDate()<o.getDate())&&--c,c>=18?`LIBRE`:c>=15?`SUB 18`:c>=12?`SUB 15`:`SUB 12`}function c(t,n,r){let i=(e.reportsBase||``).trim();if(!i)return`#`;let a=i.endsWith(`/`)?i:`${i}/`,o;try{o=new URL(t.replace(/^\//,``),a)}catch{return`#`}let s=new URL(window.location.href);return[`embedded`,`fvd_master_embed`,`torneo_id`,`ctx_torneo`,`campeonato_id`,`ret`,`return`,`fvd_ws`].forEach(e=>{let t=s.searchParams.get(e);t!==null&&t!==``&&o.searchParams.set(e,t)}),n&&r>0&&o.searchParams.set(`asociacion_id`,String(r)),o.searchParams.has(`ret`)||o.searchParams.set(`ret`,s.pathname+(s.search||``)),o.pathname+o.search}function l(e,t){if(!e)return``;let n=e?.foto_url?String(e.foto_url):``,a=n?`<img src="${r(n)}" alt="" class="mx-auto h-10 w-10 rounded-lg object-cover ring-1 ring-slate-200/80 shadow-sm" />`:`<span class="mx-auto flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-[10px] font-bold text-slate-500">${r(i(String(e?.nombre??``)))}</span>`,s=Number(e?.numfvd??0),c=s>0?String(s):`—`,l=e?.cedula!=null&&String(e?.cedula).trim()!==``?String(e?.cedula):`—`,u=r(l),d=`<div class="flex max-w-[13rem] items-center gap-1.5 whitespace-nowrap text-left text-[13px] font-semibold leading-tight text-slate-800" title="${e?.identidad_label!=null&&String(e?.identidad_label).trim()!==``?r(String(e?.identidad_label).replace(/\n/g,` · `)):`C.I. ${u}`}">
          ...
          </div>`:``,j=t.isFvdAdmin&&Array.isArray(t.asociaciones)?`<option value="0" ${d.asociacionFilter===0?`selected`:``}>Todas las asociaciones</option>${t.asociaciones.map(e=>`<option value="${e.id}" ${d.asociacionFilter===Number(e.id)?`selected`:``}>${r(e.nombre||``)}</option>`).join(``)}`:``,M=x?d.modal===`view`?`<div class="min-w-0">
            <span class="${y}">Estatus</span>
            <div class="flex min-h-[3.3rem] max-w-[12rem] items-center rounded-lg border-2 border-slate-700 bg-white px-3 text-sm font-semibold text-slate-950">${r(o(Number(x.estatus),Number(x.numfvd??0)))}</div>
          </div>`:d.modal===`edit`&&t.isFvdAdmin?`<label class="min-w-0">
            <span class="${y}">Estatus</span>
            <select name="estatus" class="${v}" ${f}>
              <option value="0" ${Number(x.estatus)===0?`selected`:``}>Pendiente</option>
              <option value="1" ${Number(x.estatus)===1?`selected`:``}>Activo</option>
              <option value="2" ${Number(x.estatus)===2?`selected`:``}>Baja</option>
              <option value="990" ${Number(x.estatus)===990?`selected`:``}>Pend. aprob. FVD</option>
            </select>
          ...
          </div>`:`<input type="hidden" name="estatus" value="${r(String(x.estatus??0))}" />`:``,re=x?s(String((x.fechnac||``).toString().slice(0,10))):`— (indique fecha de nac.)`,ie=x&&d.modal===`create`?`
      <form id="fvd-atleta-form" class="mx-auto max-w-[56rem] space-y-4 rounded-xl border-2 border-[#fff200]/80 bg-slate-100 p-4 text-slate-950 shadow-md sm:p-5">
        ${t.isFvdAdmin?`<label class="block">
            ...
            <select name="asociacion" required class="${v} w-full" ${f}>
              ...
              ${(t.asociaciones||[]).map(e=>`<option value="${e.id}" ${Number(x.asociacion)===Number(e.id)?`selected`:``}>${r(e.nombre||``)}</option>`).join(``)}
            </select>
          </label>`:`<input type="hidden" name="asociacion" value="${r(String(x.asociacion??``))}" />`}

        <div class="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_min(273px,min(42vw,100%))] lg:items-start">
          <div class="min-w-0 space-y-3">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,3fr)_minmax(0,7fr)] sm:items-start sm:gap-3">
              <div class="min-w-0 space-y-1.5">
                <label class="block">
                  <span class="${y}">Cédula</span>
                  <input name="cedula" id="fvd-atleta-cedula" required class="${v} w-full max-w-full sm:max-w-[60%]" value="${r(String(x.cedula??``))}" ${f} />
                </label>
                ${O?`<p class="text-xs font-semibold text-slate-800" role="status">${r(O)}</p>`:``}
                ${a?``:`<p class="text-[11px] leading-snug text-slate-600">Si la cédula ya está en el sistema, aparece el aviso con el estatus del registro existente (sin salir de esta pantalla).</p>`}
              </div>
              <div class="min-w-0">
                <label class="block">
                  <span class="${y}">Nombre</span>
                  <input name="nombre" id="fvd-atleta-nombre" required class="${v} w-full" value="${r(String(x.nombre??``))}" ${f} />
                </label>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <label class="min-w-0">
                ...
                <select name="sexo" class="${v}" ${f}>
                  <option value="0" ${Number(x.sexo)===0?`selected`:``}>— No indicado —</option>
                  <option value="1" ${Number(x.sexo)===1?`selected`:``}>Masculino</option>
                  <option value="2" ${Number(x.sexo)===2?`selected`:``}>Femenino</option>
                </select>
              </label>
              <label class="min-w-0">
                <span class="${y}">Fecha nac.</span>
                <input type="date" name="fechnac" id="fvd-atleta-fechnac" class="${v}" value="${r(String((x.fechnac||``).toString().slice(0,10)))}" ${f} />
              </label>
              <label class="min-w-0">
                <span class="${y}">Profesión</span>
                <input name="profesion" class="${v}" value="${r(String(x.profesion??``))}" ${f} />
              </label>
            </div>

            <label class="block">
              <span class="${y}">Dirección</span>
              <input name="direccion" class="${v} w-full" value="${r(String(x.direccion??``))}" ${f} />
            </label>

            <div class="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_minmax(0,6.5rem)] sm:items-end">
              <label class="min-w-0">
                <span class="${y}">Celular</span>
                <input name="celular" class="${v}" value="${r(String(x.celular??``))}" ${f} />
              </label>
              <label class="min-w-0">
                <span class="${y}">Email</span>
                <input type="email" name="email" class="${v}" value="${r(String(x.email??``))}" ${f} />
              </label>
              ${M}
            </div>
          </div>

          <div class="flex min-w-0 flex-col gap-2 lg:max-w-none" aria-label="Fotos carnet y documento">
            ${a?``:`<div>
              <span class="${y}">Archivo de foto del atleta (carnet)</span>
              <input type="file" id="fvd-atleta-foto-inp" name="foto" accept="image/*" class="mt-1 w-full text-xs font-medium text-slate-950 file:mr-2 file:rounded-md file:border-2 file:border-slate-600 file:bg-white file:px-2 file:py-2 file:text-xs file:font-semibold" />
            </div>`}
            <div class="relative flex min-h-[9.75rem] w-full items-center justify-center overflow-hidden rounded-lg border-2 border-slate-600 bg-slate-50 p-2 shadow-inner">
              <img id="fvd-atleta-foto-prev" src="${D?r(E):``}" alt="" class="max-h-[182px] max-w-full rounded-md object-contain ${D?``:`hidden`}" decoding="async" />
              <span id="fvd-atleta-foto-ph" class="text-center text-xs font-medium text-slate-500 ${D?`hidden`:``}">Vista previa foto carnet</span>
            </div>
            <div class="space-y-1">
              ${a?``:`<span class="${y}">Archivo imagen de cédula</span>
              <input type="file" id="fvd-atleta-cedula-inp" name="cedula_img" accept="image/*" class="w-full text-xs font-medium text-slate-950 file:mr-2 file:rounded-md file:border-2 file:border-slate-600 file:bg-white file:px-2 file:py-2 file:text-xs file:font-semibold" />`}
              <div class="flex min-h-[4rem] w-full items-center justify-center overflow-hidden rounded-lg border border-slate-500 bg-slate-50 p-2">
                <img id="fvd-atleta-cedula-prev" src="" alt="" class="hidden max-h-none w-full rounded-md object-contain" decoding="async" />
                <span id="fvd-atleta-cedula-ph" class="text-center text-xs font-medium text-slate-500">Vista previa imagen de cédula</span>
              </div>
            </div>
            <div class="flex flex-row flex-nowrap gap-2">
              <div class="min-w-0 flex-1 rounded-md border border-slate-500 bg-slate-200/80 px-2 py-2 text-xs text-slate-800">
                <span class="block font-bold uppercase tracking-wide text-slate-600">Nº FVD</span>
                <strong class="text-base text-amber-700">${Number(x.numfvd)>0?r(String(x.numfvd)):`—`}</strong>
                ${Number(x.numfvd)<=0?`<div class="mt-0.5 text-[10px] font-semibold leading-tight text-slate-600">Pendiente asignación</div>`:``}
              </div>
              <div class="min-w-0 flex-1 rounded-md border border-slate-400 bg-slate-200/60 px-2 py-2 text-xs text-slate-800">
                <span class="mb-0.5 block text-[10px] font-bold uppercase text-slate-600">Categoría (edad)</span>
                <p class="m-0 text-sm font-bold text-amber-700" id="fvd-atleta-categ-hint-txt">${r(re)}</p>
              </div>
            </div>
          </div>
        </div>
      </form>
    `:``,ae=!x||d.modal===`create`?``:`<div class="grid grid-cols-1 gap-3 sm:grid-cols-12 sm:gap-x-3 sm:gap-y-2">
          <div class="sm:col-span-2">
            <span class="${y}">Foto</span>
            <div class="relative mx-auto flex aspect-[3/4] max-h-28 w-full max-w-[7rem] items-center justify-center overflow-hidden rounded-xl border-2 border-slate-700 bg-white shadow-sm sm:mx-0">
              <img id="fvd-atleta-foto-prev" src="${D?r(E):``}" alt="" class="max-h-full max-w-full object-cover ${D?``:`hidden`}" decoding="async" />
              <span id="fvd-atleta-foto-ph" class="text-lg font-bold text-slate-600 ${D?`hidden`:``}">${r(ne)}</span>
            </div>
          ...
          <div class="sm:col-span-5 min-w-0">
            <label class="block">
              <span class="${y}">Cédula</span>
              <input name="cedula" id="fvd-atleta-cedula" required class="${v}" value="${r(String(x?.cedula??``))}" ${f} />
            </label>
          ...
          <label class="sm:col-span-3 min-w-0 sm:col-start-auto">
            ...
            <input name="numfvd_display" class="${v} bg-slate-100" readonly disabled value="${r(String(x?.numfvd??0))}" />
          </label>
        </div>`,oe=x?d.modal===`create`?ie:`
      <form id="fvd-atleta-form" class="max-w-2xl space-y-4 rounded-xl border-2 border-slate-600 bg-slate-100 p-5 text-slate-950 shadow-md">
        <input type="hidden" name="id" value="${r(String(x.id??``))}" />
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label class="min-w-0">
            <span class="${y}">Nombres</span>
            <input name="nombres" id="fvd-atleta-nombres" class="${v}" value="${r(String(x.nombres??``))}" ${f} />
          </label>
          <label class="min-w-0">
            <span class="${y}">Apellidos</span>
            <input name="apellidos" id="fvd-atleta-apellidos" class="${v}" value="${r(String(x.apellidos??``))}" ${f} />
          </label>
        </div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label class="min-w-0">
            <span class="${y}">Sexo</span>
            <select name="sexo" class="${v}" ${f}>
              <option value="0" ${Number(x.sexo)===0?`selected`:``}>—</option>
              <option value="1" ${Number(x.sexo)===1?`selected`:``}>Masculino</option>
              <option value="2" ${Number(x.sexo)===2?`selected`:``}>Femenino</option>
            </select>
          </label>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label class="min-w-0">
            <span class="${y}">Dirección</span>
            <input name="direccion" class="${v} w-full max-w-full" value="${r(String(x.direccion??``))}" ${f} />
          </label>
        ...
        ${t.isFvdAdmin?`<label class="block max-w-2xl">
            <span class="${y}">Asociación</span>
            <select name="asociacion" class="${v} w-full max-w-full sm:max-w-[min(28rem,100%)]" ${f}>
              <option value="">—</option>
              ${(t.asociaciones||[]).map(e=>`<option value="${e.id}" ${Number(x.asociacion)===e.id?`selected`:``}>${r(e.nombre||``)}</option>`).join(``)}
            </select>
          </label>`:`<input type="hidden" name="asociacion" value="${r(String(x.asociacion??``))}" />`}
      </form>
    `:`<p class="py-6 text-center text-sm font-bold text-[#2e3092]">Cargando…</p>`,se=d.tipo===`bajas`?`Bajas`:d.tipo===`no_activos`?`Pendientes`:d.tipo===`ultimos`?`Últimos mov.`:`Activos`,N=`Todas`;if(t.isFvdAdmin&&d.asociacionFilter>0&&Array.isArray(t.asociaciones)){let e=t.asociaciones.find(e=>Number(e.id)===d.asociacionFilter);N=e&&e.nombre?String(e.nombre):`Asoc. #`+d.asociacionFilter}else !t.isFvdAdmin&&Array.isArray(t.asociaciones)&&t.asociaciones[0]&&(N=String(t.asociaciones[0].nombre||`Mi asociación`));let P=d.rows.map(e=>l(e,t)).join(``),F=d.modal!==null,I=d.modal===`create`?`Nuevo atleta`:d.modal===`edit`?`Editar atleta`:d.modal===`view`?`Ficha atleta`:``,L=t.canCreate&&!F?`<button type="button" id="fvd-atleta-nuevo" class="inline-flex h-8 shrink-0 items-center justify-center whitespace-nowrap rounded-md bg-[#fff200] px-2.5 text-[10px] font-extrabold uppercase tracking-wide text-[#0b1535] shadow-md ring-1 ring-black/10 transition hover:brightness-105 sm:h-9 sm:px-3 sm:text-xs">Nuevo atleta</button>`:``,R=(e.returnUrl||``).trim(),z=`inline-flex h-8 shrink-0 items-center justify-center gap-0.5 whitespace-nowrap rounded-md border border-white/30 bg-white/10 px-2 text-[10px] font-extrabold uppercase tracking-wide text-white shadow hover:bg-white/20 sm:h-9 sm:gap-1 sm:px-2.5 sm:text-xs`,B=`${n||!R?``:`<a href="${r(R)}" class="${z}">← Regresar</a>`}${n||R?``:`<button type="button" id="fvd-atleta-regresar" class="${z}">← Regresar</button>`}${L}`,ce=B.trim()===``?``:`<div class="flex min-w-0 flex-nowrap items-center justify-end gap-1.5 overflow-x-auto border-b border-white/15 py-2 sm:gap-2">${B}</div>`,le=d.error?`<div class="border-t border-white/10 py-2"><div class="rounded-md border border-red-400/40 bg-red-950/40 px-3 py-2 text-xs font-medium text-red-100 shadow-inner" role="alert">${r(d.error)}</div></div>`:``,V=e.reportsEnabled===!0&&(e.reportsBase||``).trim()!==``,ue=V?c(`reporte_indicadores.php?marcador=afiliacion`,t.isFvdAdmin,d.asociacionFilter):`#`,de=V?c(`reporte_carnets.php`,t.isFvdAdmin,d.asociacionFilter):`#`,fe=V?c(`reporte_indicadores.php?marcador=afiliacion_anualidad`,t.isFvdAdmin,d.asociacionFilter):`#`,pe=V?c(`reporte_traspasos.php`,t.isFvdAdmin,d.asociacionFilter):`#`,me=V?`<div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1.5 border-t border-white/10 pt-2" role="navigation" aria-label="Informes HTML">
          <span class="w-full text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-500 sm:mr-1 sm:w-auto">Informes</span>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(ue)}" title="Filas con atletas.afiliacion = 1">Afiliación</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(de)}" title="Solo filas con atletas.carnet = 1">Carnets</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(fe)}" title="Afiliación y anualidad">Afil.+anual.</a>
          <a class="rounded border border-white/20 bg-white/5 px-2 py-1 text-[11px] font-semibold text-[#fff200] shadow-sm hover:bg-white/10" href="${r(pe)}" title="Historial de traspasos">Traspasos</a>
        </div>`:``,H=u.querySelector(`#fvd-atleta-q`);H instanceof HTMLInputElement&&document.activeElement===H&&(h=!0,g=H.selectionStart??H.value.length,_=H.selectionEnd??H.value.length);let he=`
            ...
            <div class="border-b border-white/10 py-2 sm:py-2.5">
              <div class="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                ...
                <div class="flex w-full min-w-0 flex-1 flex-row flex-wrap items-center gap-x-2 gap-y-1 sm:w-1/2 sm:max-w-[50%] sm:justify-end">
                  ...
                  <span class="inline-flex max-w-[min(100%,11rem)] items-center gap-1.5 truncate rounded-md border border-white/15 bg-black/30 px-2 py-0.5 shadow-inner sm:max-w-[13rem]" title="${r(N)}">
                    <span class="shrink-0 text-[9px] font-bold uppercase tracking-wide text-slate-400">Asoc.</span>
                    <span class="truncate text-xs font-semibold leading-none text-slate-100">${r(N)}</span>
                  </span>
                  <span class="inline-flex items-center gap-1.5 rounded-md border border-white/15 bg-black/30 px-2 py-0.5 shadow-inner">
                    ...
                    <span class="text-xs font-semibold leading-none text-slate-100">${r(se)}</span>
                  </span>
          ...
          <div class="fvd-atleta-tbody-scroll min-h-[min(12rem,calc(100dvh-17.5rem))] flex-1 overflow-auto rounded-b-lg border border-t-0 border-slate-300/90 bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]" style="max-height:calc(100dvh - 18.25rem)">
            <table class="min-w-[28.8rem] w-full table-fixed border-collapse text-left text-sm">
              ...
              <tbody class="divide-y divide-slate-100 bg-white text-slate-800">
                ${d.loading?`<tr><td colspan="6" class="px-3 py-12 text-center text-sm text-slate-500">Actualizando listado…</td></tr>`:d.rows.length===0?`<tr><td colspan="6" class="px-3 py-12 text-center text-sm text-slate-500">Sin resultados.</td></tr>`:P}
              </tbody>
            </table>
          </div>
      ...
      ${F?`<div class="absolute inset-0 z-[99999] flex items-center justify-center p-3 sm:p-5" role="dialog" aria-modal="true">
        ...
        <div class="relative z-10 flex max-h-[min(92dvh,90vh)] w-full max-w-[min(96vw,80vw)] flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
          <header class="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-[#1a1f5c] via-[#2e3092] to-[#3d42c4] px-4 py-3 sm:px-5">
            <div class="min-w-0">
              <h2 class="text-lg font-bold tracking-tight text-white">${r(I)}</h2>
          ...
          <footer class="flex shrink-0 flex-wrap justify-end gap-2 border-t border-slate-100 bg-white px-4 py-3 sm:px-6">
            ${d.modal===`view`&&x&&d.modalCanEdit?`<button type="button" id="fvd-atleta-to-edit" class="rounded-lg bg-[#2e3092] px-4 py-2 text-sm font-bold text-white shadow hover:bg-[#252f7a]">Editar</button>`:``}
    ...
    `;u.innerHTML=``,u.innerHTML=he;function U(){let e=u.querySelector(`#fvd-atleta-q`),t=u.querySelector(`#fvd-atleta-tipo`),n=u.querySelector(`#fvd-atleta-asoc`);d.q=e?e.value.trim():``,d.tipo=t?t.value:`normal`,n&&(d.asociacionFilter=parseInt(n.value||`0`,10)||0)}let W=u.querySelector(`#fvd-atleta-q`),G=()=>{m!=null&&clearTimeout(m),m=window.setTimeout(()=>{m=null,U(),d.page=1,b()},400)};W?.addEventListener(`input`,G),W?.addEventListener(`change`,G),W?.addEventListener(`search`,()=>{U(),d.page=1,b()}),u.querySelector(`#fvd-atleta-tipo`)?.addEventListener(`change`,()=>{U(),d.page=1,b()}),u.querySelector(`#fvd-atleta-asoc`)?.addEventListener(`change`,()=>{U(),d.page=1,b()}),u.querySelector(`#fvd-atleta-prev`)?.addEventListener(`click`,()=>{d.page>1&&(--d.page,b())}),u.querySelector(`#fvd-atleta-next`)?.addEventListener(`click`,()=>{d.page<d.pages&&(d.page+=1,b())}),u.querySelectorAll(`[data-view]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=parseInt(e.getAttribute(`data-view`)||`0`,10);t&&await w(`view`,t)})}),u.querySelectorAll(`[data-edit]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=parseInt(e.getAttribute(`data-edit`)||`0`,10);t&&await w(`edit`,t)})}),u.querySelectorAll(`[data-toggle-atleta]`).forEach(e=>{e.addEventListener(`change`,e=>{let t=e.target,n=parseInt(t.getAttribute(`data-toggle-atleta`)||`0`,10);n&&T(n)})}),u.querySelectorAll(`[data-close-modal]`).forEach(e=>{e.addEventListener(`click`,()=>C())}),u.querySelector(`#fvd-atleta-to-edit`)?.addEventListener(`click`,async()=>{let e=d.formRow&&d.formRow.id?parseInt(String(d.formRow.id),10):0;e&&await w(`edit`,e)}),u.querySelector(`#fvd-atleta-save`)?.addEventListener(`click`,()=>{let e=u.querySelector(`#fvd-atleta-form`);e&&ee(e)});let K=u.querySelector(`#fvd-atleta-cedula`);K?.addEventListener(`input`,()=>{d.modal!==`create`||!K||(p!=null&&clearTimeout(p),p=window.setTimeout(()=>{p=null,S(K.value)},450))}),K?.addEventListener(`blur`,()=>{d.modal!==`create`||!K||(p!=null&&(clearTimeout(p),p=null),S(K.value))});let q=u.querySelector(`#fvd-atleta-foto-inp`),J=u.querySelector(`#fvd-atleta-foto-prev`),ge=u.querySelector(`#fvd-atleta-foto-ph`);q?.addEventListener(`change`,()=>{let e=q.files&&q.files[0];if(!e||!J)return;let t=new FileReader;t.onload=()=>{J.src=typeof t.result==`string`?t.result:``,J.classList.remove(`hidden`),ge?.classList.add(`hidden`)},t.readAsDataURL(e)});let Y=u.querySelector(`#fvd-atleta-cedula-inp`),X=u.querySelector(`#fvd-atleta-cedula-prev`),_e=u.querySelector(`#fvd-atleta-cedula-ph`);Y?.addEventListener(`change`,()=>{let e=Y.files&&Y.files[0];if(!e||!X)return;let t=new FileReader;t.onload=()=>{X.src=typeof t.result==`string`?t.result:``,X.classList.remove(`hidden`),_e?.classList.add(`hidden`)},t.readAsDataURL(e)});let Z=u.querySelector(`#fvd-atleta-fechnac`),Q=u.querySelector(`#fvd-atleta-categ-hint-txt`),$=()=>{d.modal!==`create`||!Z||!Q||(Q.textContent=s(Z.value))};Z?.addEventListener(`change`,$),Z?.addEventListener(`input`,$),h&&(h=!1,requestAnimationFrame(()=>{let e=u.querySelector(`#fvd-atleta-q`);if(!(e instanceof HTMLInputElement))return;e.focus();let t=e.value.length,n=Math.min(Math.max(0,g),t),r=Math.min(Math.max(0,_),t);try{e.setSelectionRange(n,r)}catch{}}))}E(),b()}u();