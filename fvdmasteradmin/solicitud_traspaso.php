...
$fvd_delegado_sol_redirect = $fvdAppBasePath . '/fvdmasteradmin/solicitud_traspaso.php';
$fvd_delegado_sol_tipo_requerido = 'traspaso';
...
<div class="fvd-dash" style="max-width:40rem">
    <h1>Solicitar traspaso de asociación</h1>
    <p style="font-size:0.8125rem;color:var(--fvd-muted);margin:0 0 1rem">Envío a la FVD para revisión. No sustituye la gestión en el módulo de atletas.</p>
    ...
    <section class="fvd-card" style="padding:12px;margin-top:10px">
        <form method="post" action="">
            ...
            <select class="fvd-input" name="asociacion_destino_id"<?= $asocDestino === [] ? ' disabled' : ' required' ?> style="width:100%;max-width:22rem;margin-bottom:8px">
                <?php if ($asocDestino === []): ?>
                    <option value="">No hay otras asociaciones</option>
                <?php endif; ?>
                <?php foreach ($asocDestino as $ad): ?>
                    <option value="<?= (int) ($ad['id'] ?? 0) ?>"><?= htmlspecialchars((string) ($ad['nombre'] ?? ''), ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
            </select>
            ...
            <button type="submit" class="fvd-btn-primary"<?= $asocDestino === [] ? ' disabled' : '' ?>>Enviar solicitud</button>
        </form>
    </section>
</div>
<?php
